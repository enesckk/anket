  ...
  if (state.activeModal.type === 'review_survey') {
    ...
    return `
      <div class="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 max-w-2xl w-full shadow-2xl space-y-5 max-h-[88vh] flex flex-col animate-in zoom-in-95 duration-150 border-none">
          
          <!-- MODAL SÜPER PREMİUM KURUMSAL HEADER -->
          <div class="flex justify-between items-start pb-4 border-b border-slate-100 shrink-0">
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-[#01214A] text-white shadow-xs">ŞEHİTKAMİL SGM</span>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-amber-100 text-amber-900 border border-amber-300">⏳ ONAY İNCELEMESİ</span>
              </div>
              <h3 class="text-lg font-extrabold text-[#01214A] tracking-tight mt-1">${survey.title}</h3>
              <p class="text-xs text-slate-500 font-medium">${survey.description || 'Açıklama belirtilmedi'}</p>
              <span class="text-[10px] text-slate-400 font-semibold block">Oluşturan Personel: <strong class="text-[#01214A]">${survey.createdBy || 'Saha Görevlisi'}</strong></span>
            </div>
            <button type="button" id="btn-close-custom-modal" class="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-all">
          ...
          <div class="flex-1 overflow-y-auto space-y-4 pr-1">
            <div class="flex items-center justify-between">
              <h4 class="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Soru Bazlı Detaylı Kontrol Listesi (${questions.length} Soru)</h4>
              <span class="text-[10px] text-[#00A0DF] font-bold">Her soruyu ayrı değerlendirebilirsiniz</span>
            </div>
            
            ${questions.length === 0 ? `
              <div class="p-6 bg-slate-50 rounded-2xl text-center text-xs text-slate-500 font-medium border border-dashed border-slate-200">Bu ankette henüz soru tanımlanmamıştır.</div>
            ` : questions.map((q, idx) => `
              <div class="p-5 bg-white rounded-2xl shadow-card hover-lift space-y-3.5 border-none">
                <div class="flex justify-between items-start gap-2">
                  <div class="space-y-1">
                    <span class="px-2 py-0.5 rounded-md bg-[#00A0DF]/10 text-[#00A0DF] text-[10px] font-extrabold uppercase">Soru ${idx + 1} • ${formatQuestionType(q.type)}</span>
                    <h5 class="font-extrabold text-[#01214A] text-xs leading-snug">${q.title}</h5>
                  </div>
                  <div class="shrink-0">
                    ${q.reviewStatus === 'APPROVED' ? `<span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-800 inline-flex items-center gap-1">${iconSvg('checkCircle', 'w-3.5 h-3.5 text-emerald-600')} Soruda Hata Yok</span>` : ''}
                    ${q.reviewStatus === 'REVISION_REQUESTED' ? `<span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-orange-100 text-orange-900 inline-flex items-center gap-1">${iconSvg('edit', 'w-3.5 h-3.5 text-orange-700')} Revizyon İstendi</span>` : ''}
                    ${q.reviewStatus === 'REJECTED' ? `<span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-red-100 text-red-800 inline-flex items-center gap-1">${iconSvg('block', 'w-3.5 h-3.5 text-red-600')} Soru Çıkarılsın</span>` : ''}
                    ${(!q.reviewStatus || q.reviewStatus === 'PENDING') ? `<span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-slate-100 text-slate-700 inline-flex items-center gap-1">${iconSvg('history', 'w-3.5 h-3.5 text-slate-500')} İncelenmedi</span>` : ''}
                  </div>
                </div>

                ${(q.options && q.options.length > 0) ? `
                  <div class="flex flex-wrap gap-1.5 pt-1">
                    ${q.options.map(opt => `<span class="px-2.5 py-1 bg-slate-50 border border-slate-200/80 rounded-lg text-[10px] text-slate-700 font-semibold">${opt.label}</span>`).join('')}
                  </div>
                ` : ''}
                ...
                <div class="pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
                  <input type="text" data-survey-id="${survey.id}" data-q-id="${q.id}" value="${q.reviewNote || ''}" placeholder="Soruya özel revizyon notu giriniz (opsiyonel)..." class="input-q-review-note flex-1 h-9 px-3.5 bg-slate-50 border border-slate-200 rounded-xl text-[11px] focus:outline-none focus:border-[#2A9D38] focus:bg-white font-medium transition-all"/>
                  
                  <div class="flex items-center gap-1.5 shrink-0">
                    <button type="button" data-survey-id="${survey.id}" data-q-id="${q.id}" data-status="APPROVED" class="btn-set-q-review-status px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 font-bold text-[10px] rounded-xl transition-all inline-flex items-center gap-1 active:scale-95 shadow-xs">
                      ${iconSvg('checkCircle', 'w-3.5 h-3.5 text-emerald-600')}
                      <span>Onayla</span>
                    </button>
                    <button type="button" data-survey-id="${survey.id}" data-q-id="${q.id}" data-status="REVISION_REQUESTED" class="btn-set-q-review-status px-3 py-1.5 bg-orange-50 text-orange-700 hover:bg-orange-100 border border-orange-200 font-bold text-[10px] rounded-xl transition-all inline-flex items-center gap-1 active:scale-95 shadow-xs">
                      ${iconSvg('edit', 'w-3.5 h-3.5 text-orange-600')}
                      <span>Revize Et</span>
                    </button>
                    <button type="button" data-survey-id="${survey.id}" data-q-id="${q.id}" data-status="REJECTED" class="btn-set-q-review-status px-3 py-1.5 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 font-bold text-[10px] rounded-xl transition-all inline-flex items-center gap-1 active:scale-95 shadow-xs">
          ...
          <div class="pt-3 border-t border-slate-100 flex flex-col sm:flex-row gap-2.5 shrink-0">
            ...
            <button type="button" data-survey-id="${survey.id}" class="btn-submit-survey-revision flex-1 h-12 bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-xs rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
              ${iconSvg('edit', 'w-4 h-4 text-white')}
              <span>Revizyon Talebini İlet</span>
            </button>

            <button type="button" id="btn-close-custom-modal" class="h-12 px-5 border border-slate-200 text-slate-700 font-bold text-xs rounded-xl hover:bg-slate-50 transition-all">
              Kapat
            </button>
          </div>
        </div>
